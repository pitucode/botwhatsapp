...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	...
	try {
		if (isUrl(text)) {
			let s = await sticker(false, text, wm, wm)
			...
			if (/image|video/g.test(mime)) {
				...
				if (ch > 0) buffer = await (/webp/g.test(mime) ? addExif(img, wm, wm) : ch > 1 ? addExif(await video2webp(img, isNaN(fps) ? 15 : fps), wm, wm) : sticker(img, false, wm, wm))
				else {
					try { buffer = await addExif(await sticker3(img, false), wm, wm) }
					catch { buffer = await sticker(img, false, wm, wm) }
				}
