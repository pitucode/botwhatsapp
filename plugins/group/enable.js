...
let handler = async (m, { conn, usedPrefix, command, args, isOwner, isBotAdmin, isAdmin, isROwner }) => {
	...
	switch (type) {
		...
		case 'psgc':
			...
			break
		default:
			if (!/[01]/.test(command)) return m.reply(`*List option :*\n| presence | welcome | delete | antidelete | autolevelup | ephemeral | nsfw | game | anticall | antilink | antivirtex | antiviewonce | autoai | autoaipc | public | self | restrict | autoread | pconly | gconly | rpg|
