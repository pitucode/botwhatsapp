		...
		for (let name in plugins) {
			...
			if ((usedPrefix = (match[0] || '')[0])) {
				...
				if (!isPrems && plugin.limit && db.data.users[m.sender].limit < plugin.limit * 1) {
					this.reply(m.chat, `Limit anda habis, silahkan claim limit melalui *${usedPrefix}claim*`, m)
...
global.dfail = (type, m, conn) => {
	let msg = {
		...
		mods: `*「DEV / MODS ONLY」*`,
		premium: `\nFitur untuk member premium!, jika ingin memakai fitur ini, silahkan membeli fitur premium dengan ketik *.owner* / *.sewa* / *.premium*\n\nUntuk melihat harga premium, ketik *.premium*`,
		group: `*「GROUP ONLY」*`,
		private: `*「PRIVATE CHAT ONLY」*`,
		admin: `*「HANYA ADMIN GROUP / PENYEWA BOT!」*`,
		nsfw: `[ *NSFW GAK AKTIF* ]`,
		game: '```「 Mode Game belum diaktifkan oleh owner 」```',
		rpg: '```「 Mode RPG belum diaktifkan oleh owner! 」```',
